...
def manifest_tester_no_secret_charm() -> Path:
    ...
    charm_path = Path.cwd() / "tests/integration/manifests-tester-no-secret"
    if not (path := next(iter(charm_path.glob("*.charm")), None)):
        logger.warning(
            "Could not find packed manifest-tester (no secret) charm. Building one now..."
        )
        subprocess.run(["charmcraft", "pack"], check=True, cwd=charm_path)
    if not (path := next(iter(charm_path.glob("*.charm")), None)):
        raise FileNotFoundError(
            "Could neither find, nor build the manifest-tester (no secret) charm."
        )
    return path
