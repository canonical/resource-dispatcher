...
"""This test module tests the charm behavior when resource-dispatcher charm is first upgraded,
and then only the manifests-tester charm is upgraded. This is the recommended and correct way to
upgrade the charms, because the resource-dispatcher that uses kubernetes_manifests 0.1 won't work
with the manifests-tester that uses kubernetes_manifests 0.2.
"""
...
TESTER_CONFIGMAPS_NAMES = ["test1-configmap"]
...
def test_integrate_tester_with_resource_dispatcher(juju: jubilant.Juju):
    ...
    juju.integrate(f"{RESOURCE_DISPATCHER_CHARM_NAME}:secrets", f"{MANIFEST_TESTER_CHARM}:secrets")
    juju.integrate(
        f"{RESOURCE_DISPATCHER_CHARM_NAME}:service-accounts",
        f"{MANIFEST_TESTER_CHARM}:service-accounts",
    )
    juju.integrate(
        f"{RESOURCE_DISPATCHER_CHARM_NAME}:pod-defaults", f"{MANIFEST_TESTER_CHARM}:pod-defaults"
    )
    status = juju.wait(
        lambda status: jubilant.all_active(status) and jubilant.all_agents_idle(status), delay=5
    )
...
def test_upgrade_resource_dispatcher(juju: jubilant.Juju, resource_dispatcher_charm: Path):
    ...
    juju.refresh(
        app=RESOURCE_DISPATCHER_CHARM_NAME, path=resource_dispatcher_charm, resources=resources
    )
...
def test_upgrade_tester_charm(juju: jubilant.Juju, manifest_tester_charm: Path):
    ...
    # The charms should eventually settle to active idle state
    status = juju.wait(
        lambda status: jubilant.all_active(status) and jubilant.all_agents_idle(status), delay=5
    )
    assert (
    ...
    assert (
        status.apps[MANIFEST_TESTER_CHARM]
        .units[f"{MANIFEST_TESTER_CHARM}/0"]
        .workload_status.current
        == "active"
    )
...
def test_integrate_new_relations_with_resource_dispatcher(
    ...
    juju.integrate(f"{RESOURCE_DISPATCHER_CHARM_NAME}:roles", f"{MANIFEST_TESTER_CHARM}:roles")
    juju.integrate(
...
def test_change_in_manifest_reflected_again(
    ...
    """Change the config in manifest-tester, such that the manifests are changed, and verify they are reflected in the K8s resources."""
    juju.config(MANIFEST_TESTER_CHARM, {"service_account_name": SERVICE_ACCOUNT_NAME_3})
    juju.wait(
        lambda status: jubilant.all_active(status) and jubilant.all_agents_idle(status), delay=10
    )
    ...
    assert_resource_status(
        lightkube_client, ServiceAccount, SERVICE_ACCOUNT_NAME_2, namespace, exists=False
    )
    assert_resource_status(lightkube_client, ServiceAccount, SERVICE_ACCOUNT_NAME_3, namespace)


def test_profile_scoped_secrets(
    lightkube_client: lightkube.Client, profile_namespaces: tuple[str, str]
):
